 ...
 process:  /**now process the text.  */
  ...
  nlen= validate_utf8_text(ntext, len);
	if(nlen){
		...
		if (trim_clever) {
            // https://stackoverflow.com/questions/6946064/gtk-regexp-how-to-get-groups-value
			...
			g_fprintf(stderr, "====remove_leading_empy_lines====:\n_%s_\n==========\n",s);
			g_regex_unref(regex);
			...
			// g_strlcpy(ntext, s, g_utf8_strlen(s,-1));
			// glong i = g_utf8_strlen(*s,-1);
			
			// g_fprintf(stderr, "i: %l\n", i);
			//g_strlcpy(*ntext, (gchar) *s, i);
